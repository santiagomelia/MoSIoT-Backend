...
{
public class IMAppointmentAssemblerDTO {
public static IList<IMAppointmentEN> ConvertList (IList<IMAppointmentDTO> lista)
{
        IList<IMAppointmentEN> result = new List<IMAppointmentEN>();
        foreach (IMAppointmentDTO dto in lista) {
...
public static IMAppointmentEN Convert (IMAppointmentDTO dto)
{
        IMAppointmentEN newinstance = null;

        try
        {
                if (dto != null) {
                        newinstance = new IMAppointmentEN ();



                        newinstance.Date = dto.Date;
                        if (dto.Appointment_oid != -1) {
                                MoSIoTGenNHibernate.CAD.MosIoT.IAppointmentCAD appointmentCAD = new MoSIoTGenNHibernate.CAD.MosIoT.AppointmentCAD ();

                                newinstance.Appointment = appointmentCAD.ReadOIDDefault (dto.Appointment_oid);
                        }
